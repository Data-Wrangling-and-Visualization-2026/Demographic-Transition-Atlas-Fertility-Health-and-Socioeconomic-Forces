...
def build_indicator_query(indicators):
    parts = []

    for code in indicators:
        parts.append(f"""
            COUNT({code}) AS {code}_non_null_points,
            COUNT(DISTINCT country_iso3) FILTER (WHERE {code} IS NOT NULL) AS {code}_countries_with_data,
            MIN(year) FILTER (WHERE {code} IS NOT NULL) AS {code}_min_year,
            MAX(year) FILTER (WHERE {code} IS NOT NULL) AS {code}_max_year,
            MIN({code}) AS {code}_min_value,
            MAX({code}) AS {code}_max_value
        """)
...
def register_routes(app: FastAPI):
    ...
    def root():
        return {
            "message": "Demographic Transition Atlas API",
            "docs": "/docs"
        }
    ...
    def list_indicators():
        try:
            with engine.connect() as conn:
                ...
                query = build_indicator_query(INDICATORS.keys())

                stats = conn.execute(text(query)).mappings().one()
                ...
                for code, meta in INDICATORS.items():
                    out.append({
                        ...
                        "non_null_points": stats[f"{code}_non_null_points"],
                        "countries_with_data": stats[f"{code}_countries_with_data"],
                        "min_year": stats[f"{code}_min_year"],
                        "max_year": stats[f"{code}_max_year"],
                        "min_value": stats[f"{code}_min_value"],
                        "max_value": stats[f"{code}_max_value"],
                    })
